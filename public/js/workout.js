...
function GenerateWorkout(
  ...
  Potentialexercises.forEach(function (item, index) {
    ...
    // console.log(item.intensity >= Workoutintensity);
    if (
      UserEquipmentList.includes(item.equipment_required) &&
      item.intensity <= Workoutintensity
    ) {
  ...
  while (Numberofexercises > 0) {
    WorkOutList.push(
      FilteredExcersizeList[
        Math.floor(Math.random() * (FilteredExcersizeList.length - 0) + 0)
      ]
    );
    Numberofexercises--;
  }
